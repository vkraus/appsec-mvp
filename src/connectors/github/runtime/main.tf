...
# Overlay file paths driven by operational.yml.source_runtime.juice_shop_overlay_files.
# The bodies of these files live under runtime/files/juice-shop/ (operator-authored
# sidecar territory; never emitted by provision-source) and are referenced via
# `file("${path.module}/files/juice-shop/${each.value}")` below.
locals {
