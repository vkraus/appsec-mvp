#!/usr/bin/env bash
# Top-level orchestrator for the github connector.
# Chain: runtime/install.sh -> scripts/load-secrets.sh -> databricks bundle deploy.
# Pass --skip-runtime when the SCM platform (GitHub org + Actions wiring) is
# already provisioned and only the Databricks side needs to be installed.
set -euo pipefail
SKIP_RUNTIME=0
...
echo "OK: github connector installed."
