...
SEV_LOOKUP = yaml.safe_load(
    (REPO_ROOT / "src" / "connectors" / "dependency_track" / "severity.yml").read_text()
)
STATUS_LOOKUP = yaml.safe_load(
    (REPO_ROOT / "src" / "connectors" / "dependency_track" / "status.yml").read_text()
)
...
def test_load_lookup_reads_yaml_from_disk() -> None:
    path = REPO_ROOT / "src" / "connectors" / "dependency_track" / "severity.yml"
