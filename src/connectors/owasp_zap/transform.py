...
- ``cwe_id`` is projected from ZAP's ``cweid`` when present and not
  the placeholder ``0`` / ``-1``.
- ``cve_id`` is NULL — ZAP does not emit CVE identifiers (added in
  Phase B5 to the ``silver_findings`` schema for SCA tools).
- ``status_canonical`` is always ``"open"`` — ZAP scans are
  point-in-time snapshots with no server-side lifecycle.

Application linkage (``application_id``) is intentionally NOT resolved
here. The canonical ``url`` → ``silver.deployments`` → application
join runs at the Silver-layer aggregator (see
``mkdocs/docs/connectors/dast/owasp-zap.md`` § Reference > Quirks),
not in the per-connector transform.

The module exposes three shapes so the framework contract, the
silver-projection logic, and the dedup-key builder can each be
...
# ZAP placeholder cweid values that mean "no CWE classification".
...
def _parse_zap_generated_at(raw: str | None) -> datetime | None:
    ...
    if raw is None:
        return None
    # Try the most-common ZAP report formats.
    for fmt in (
        "%a, %d %b %Y %H:%M:%S",
        "%Y-%m-%dT%H:%M:%SZ",
        "%Y-%m-%dT%H:%M:%S",
...
def _normalize_cwe(raw: Any) -> str | None:
    ...
    (``0``, ``-1``). All other values are coerced to a string. The
    enrichment side-table (``src/platform/cwe.py``) decorates the
...
def dedup_key_for(
    site: str | None, alert_id: str | None, uri: str | None
) -> tuple[str | None, str | None, str | None]:
    """Return the dedup tuple ``(target, alert_id, uri_path)`` for a ZAP
    finding, per the DAST category reference (``references/dast.md`` §
    "Deduplication key") and ``mapping.yml``'s ``dedup_key`` block.

    The tuple is the join basis for ``dedup_links`` in the Silver layer.
    Note: the canonical Silver ``silver.findings.url`` carries the URI
    (since DAST has no ``file_path``); the dedup tuple's ``target``
    component corresponds to the scanned site/host.
    """
    return (site, alert_id, uri)
...
def alerts_to_silver(
    ...
    *,
    ...
    lookups from ``severity.yml`` / ``status.yml``.
    ...
    for report in raw:
        ...
        for site in report.get("site") or []:
            target = site.get("@name") or site.get("@host")
            if not target:
                continue
            for alert in site.get("alerts") or []:
                ...
                if not instances:
                    # ZAP rarely emits an alert without instances, but be
                    # defensive: REQ-DQ requires we drop rows that cannot
                    # produce a valid (target, alert_id, uri) dedup key.
                    continue
                for inst in instances:
                    ...
                    rows.append(
                        Row(
                            ...
                            cwe_id=cwe,
                            cve_id=None,  # DAST emits no CVE identifiers
                            rule_id_native=plugin_id,
                            trigger_context=trigger_context,
                            # application_id resolution against
                            # silver.deployments runs at the Silver-layer
                            # aggregator, not in this connector transform.
                            repository_id=None,
