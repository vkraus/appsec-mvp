"""OWASP ZAP test configuration.

Patch PySpark's TimestampType.fromInternal to return UTC-aware datetimes
so that collected timestamps can be compared to timezone-aware Python
datetimes regardless of the machine's local timezone.

On Databricks clusters the JVM timezone is always UTC; locally it may
differ. This patch makes tests portable, matching the established
pattern in src/connectors/sonarqube/tests/conftest.py.
"""
