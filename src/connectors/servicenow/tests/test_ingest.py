...
``config.yml`` and in the canonical framework helpers in ``src/platform/``.
...
_CONFIG_PATH = Path(__file__).parents[1] / "config.yml"
_JOB_PATH = Path(__file__).parents[1] / "resources" / "job.yml"
