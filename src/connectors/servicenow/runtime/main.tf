...
#   - `terraform_data` + `local-exec curl` issues the POST calls (the
#     ServiceNow table API does not have a first-class Terraform provider,
#     so curl-via-local-exec is the native pattern for seeding demo CMDB
#     records).
#
# Sub-shape: terraform-saas-seed (provider hashicorp/http only, per
# references/cmdb.md).
# ---------------------------------------------------------------------------
