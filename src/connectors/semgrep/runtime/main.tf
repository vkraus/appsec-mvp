...
# Kubernetes auth is derived from EKS data sources so this runtime is
# self-contained (no aws-foundation cross-runtime references).
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# IRSA IAM role for the Semgrep service account. Inlined as raw IAM
# resources rather than the community
# `terraform-aws-modules/iam/aws//modules/iam-role-for-service-accounts-eks`
# module so this runtime is free of community-module pinning.
# ---------------------------------------------------------------------------
...
# The ConfigMap loads the operator-authored driver script from
# runtime/files/semgrep-scan.sh. The path is recorded in
# operational.yml.source_runtime.scan_script_path; the script body is NOT
# emitted by the provision-source skill (operator territory).
