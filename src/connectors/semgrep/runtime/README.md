...
Apply this module if you want appsec-mvp to provision a periodic Semgrep scan runner on your EKS cluster. Skip it if you already have Semgrep wired up — feed the bucket/prefix where your existing scanner writes findings directly into the connector's secrets.

## What it creates

- A `semgrep` Kubernetes namespace + service account on your EKS cluster, with an IRSA annotation pointing at the IAM role this module mints.
- An IAM role assumable by the `semgrep` service account via IRSA, granted `s3:PutObject` / `s3:GetObject` / `s3:ListBucket` on `var.artifact_bucket` and its objects (mirrors the source `module "semgrep_irsa"` + `aws_iam_policy.artifact_bucket_write` from `aws-foundation`).
...
|---|---|
| `aws_region` | AWS region of the EKS cluster + the artifact S3 bucket. Must match the region of `eks_cluster_name`. |
...
| `project_prefix` | Tag/name prefix for AWS resources (IAM role name, IAM policy name, tags). | `appsec-mvp` |
...
`semgrep_namespace`, `semgrep_cronjob_name`, `semgrep_irsa_role_arn`, `semgrep_env_secret_name` — useful as inputs for connector wiring (e.g. surfacing the bucket prefix into the Semgrep connector's secrets) and for `kubectl` debugging.
...
- If a Semgrep pod is mid-scan when destroy runs, it gets terminated. The partial scan's working directory is in an `emptyDir` volume and is lost; if you care about the in-flight findings, wait for the CronJob's most recent Job to complete before destroying.
