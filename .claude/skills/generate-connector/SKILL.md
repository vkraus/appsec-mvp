...
- `src/connectors/{source}/operational.yml` exists with the `databricks_runtime:` sub-block populated per the schema in `references/<category>.md`. The skill does NOT generate this file; it is bootstrapped per-source upstream.
...
2. **Read `src/connectors/{source}/operational.yml`.** Parse the `databricks_runtime:` sub-block. Cross-check every field declared `required` in the category schema. **HALT if the file is missing OR if any required-by-category field is absent.** The halt output is a structured list of missing fields (`<field>: <description>` per row) so the controller can gather them, fix `operational.yml`, and re-run. Do not partial-emit any file.
...
- If `src/connectors/{source}/operational.yml` is missing OR has missing required-by-category fields under `databricks_runtime:`, the skill HALTS and reports the list of missing fields. Do NOT partial-emit. The controller is expected to gather the missing fields, write `operational.yml`, and re-run.
- The skill DOES NOT generate `operational.yml`. That file is bootstrapped per-source upstream (Phase 1 of the controlling plan); the skill only reads it.
