...
TruffleHog is a CLI tool with no HTTP API. The connector invokes it as a subprocess and captures standard output. The canonical invocation is:

```
trufflehog <source-kind> <source-args> --json [--results=verified,unknown]
```

Relevant source kinds: `git`, `github`, `gitlab`, `filesystem`, `docker`, `s3`. Authentication for the target source (SSH key, GitHub PAT, AWS credentials) is provided via source-kind-specific flags or environment variables. TruffleHog itself needs no separate authentication. Credentials are stored in Databricks Secrets and injected into the subprocess environment.

### Pagination and rate limits

Pagination does not apply: the invocation streams line-delimited JSON to standard output. The connector parses each object as it arrives.

Rate limits apply to the upstream source. For the `github` kind, the rate-limit behaviour described for the GitHub source applies. For S3, the connector sets `--concurrency` conservatively to stay within account quotas. Git and filesystem kinds are local and have no network constraints.

### Incremental hook

TruffleHog has no general-purpose incremental mode. Each invocation is a complete scan. The `git` kind accepts `--since-commit` to restrict the scan to commits after a checkpoint. The connector records the most recent commit SHA in the high-water mark state table and supplies it as `--since-commit` on the next run.

`filesystem`, `docker`, and `s3` kinds have no equivalent flag: each invocation rescans the full target. These fall in the full-reload category. The connector stores a synthetic last-full-scan timestamp for cadence observability only.
...
`SourceMetadata.Data` is source-kind-specific. The `Git` shape above is the primary variant. Filesystem, GitHub, and S3 scans use leaf structures `Filesystem`, `Github`, `S3`; the connector has source-kind-specific extraction logic.

### Enumerations

**DetectorName.** TruffleHog ships over 800 detectors covering AWS, GitHub, GitLab, Slack webhooks, Stripe, JIRA, Postgres, MongoDB, and hundreds more. The reference implementation stores `DetectorName` verbatim in Bronze and maps it to `secret_type` without normalisation. The full detector list lives in the TruffleHog repository.

**Verified status mapping.** `validity_status` derives from `Verified` and `VerificationError`: `Verified=true` → `active`; `Verified=false` with empty `VerificationError` → `inactive`; `Verified=false` with non-empty `VerificationError` → `unknown`. The `unknown` category matters: it covers secrets on isolated networks or against deprecated provider APIs.
