...
Authentication is account-scoped rather than per-tenant: cloud-native WAFs authenticate via IAM role or access key bound to the cloud account hosting the WebACLs, and on-prem appliances authenticate via a service credential bound to the log-aggregation tier. There is no per-application authentication axis; application scoping is derived at transform time from the WebACL's associated resource ARNs (ALB, CloudFront distribution, API Gateway stage) joined against `silver.deployments`.
