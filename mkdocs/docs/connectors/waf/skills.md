...
Three skills operationalize the connector lifecycle for WAF sources.

!!! info "Specialization pending"
    These skills will be specialized for WAF sources (renamed to
    `analyze-source-waf`, `generate-connector-waf`,
    `validate-implementation-waf`) in a follow-up work item. Until then,
