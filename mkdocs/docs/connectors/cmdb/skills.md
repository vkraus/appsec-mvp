...
Three skills operationalize the connector lifecycle for CMDB sources.

!!! info "Specialization pending"
    These skills will be specialized for CMDB sources (renamed to
    `analyze-source-cmdb`, `generate-connector-cmdb`,
    `validate-implementation-cmdb`) in a follow-up work item. Until then,
