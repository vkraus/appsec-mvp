...
Connectors ingest from AppSec source systems into the platform's Bronze
layer, then transform into the canonical Silver entities and findings.
...
for `silver.repositories`, the cross-source canonical repository entity.
Every non-SCM connector's findings carry a `repository_id` that must
...
surface — authentication, pagination, incremental strategy — and lists the
category's Claude Code skills.
