...
declarative YAML lookups in ``config/severity/gitlab.yml`` and
``config/status/gitlab.yml``.
...
_SEVERITY_PATH = _REPO_ROOT / "config" / "severity" / "gitlab.yml"
_STATUS_PATH = _REPO_ROOT / "config" / "status" / "gitlab.yml"
...
def test_project_to_repository_projects_expected_fields() -> None:
    ...
    assert by_id["200"]["updated_at"] == datetime(2026, 4, 20, 10, 0, tzinfo=timezone.utc)
...
def test_merge_request_to_pull_request_projects_expected_fields() -> None:
    ...
    assert merged["merged_at"] == datetime(2026, 4, 18, 14, 22, tzinfo=timezone.utc)
...
def test_parse_iso_utc_roundtrips_timezone_aware() -> None:
    ...
    assert zulu == datetime(2026, 4, 20, 10, 0, tzinfo=timezone.utc)
    assert offset == datetime(2026, 4, 20, 10, 0, tzinfo=timezone.utc)
