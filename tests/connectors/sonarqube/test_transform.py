...
_SEV_PATH = Path(__file__).parents[3] / "config" / "severity" / "sonarqube.yml"
_STATUS_PATH = Path(__file__).parents[3] / "config" / "status" / "sonarqube.yml"
...
def test_issue_mapping(spark: SparkSession) -> None:
    ...
    assert row["cwe_id"] is None  # enriched via src/common/cwe.py side table
...
def test_severity_undocumented_falls_through_to_default() -> None:
    ...
    ``info`` default (per ``src.common.silver.normalize_severity``) rather
...
def test_status_undocumented_falls_through_to_open() -> None:
    ...
    src.common.silver.normalize_status.
    """
    status_map = load_yaml(StatusMap, _STATUS_PATH)
...
def test_creation_date_to_utc_datetime(spark: SparkSession) -> None:
    ...
    row = out["AYx1aaaaaaaaaaaaaaa1"]
    assert row["first_seen_at"] == datetime(2026, 4, 18, 9, 0, tzinfo=timezone.utc)
    assert row["last_seen_at"] == datetime(2026, 4, 20, 10, 0, tzinfo=timezone.utc)
